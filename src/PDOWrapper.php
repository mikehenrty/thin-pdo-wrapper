...
class PDOWrapper {
	 ...
	 * 	
     * @param dsnPrefix - the dsn prefix
	 ...
	 */
	public function configMaster($dsnPrefix, $host, $name, $user, $password, $port=null) {
        if (!$this->validateDriver($dsnPrefix)) {
	 ...
	 * @param port (optional) - the port to connect using, default to 3306
	 */
	public function configSlave($dsnPrefix, $host, $name, $user, $password, $port=null) {
        if (!$this->validateDriver($dsnPrefix)) {
            throw new Exception('DATABASE WRAPPER::error, the database you wish to connect to is not supported by your install of PHP.');
        }
	 ...
	 */
	protected function createConnection($dsnPrefix, $host, $name, $user, $password, $port=null) {
        if (!$this->validateDriver($dsnPrefix)) {
            throw new Exception('DATABASE WRAPPER::error, the database you wish to connect to is not supported by your install of PHP.');
        }
		...
		try {
            ...
            if ($dsnPrefix == "sqlite" || $dsnPrefix == "sqlite2") {
                $connection_string = $dsnPrefix.':'.$host;
            } elseif ($dsnPrefix == "sqlsrv") {
                $connection_string = "sqlsrv:Server=".$host.";Database=".$name;
            } elseif ($dsnPrefix == "firebird" || $dsnPrefix == "oci") {
                $connection_string = $dsnPrefix.":dbname=".$name;
            } else {
                $connection_string = $dsnPrefix.':host='.$host.';dbname='.$name;
            }
		...
		if (!isset($this->pdo_slave)) {
			...
			if (!isset($this->config_slaves)) {
				...
				while (defined('self::SLAVE' . $i . '_HOST') 
					...
					$this->config_slaves[] = array(
                        'dsn' => constant('self::SLAVE' . $i . '_DSN_PREFIX'),
     ...
     * @param string $dsnPrefix The DSN prefix
     * @return boolean true, the database is supported; false, the database is not supported.
     */
    private function validateDriver($dsnPrefix) {
        if (!in_array($dsnPrefix, PDO::getAvailableDrivers())) {
            return false;
        } else {
            return true;
        }
